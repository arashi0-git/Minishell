...
/*   Updated: 2025/02/09 17:13:12 by aryamamo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/minishell.h"

int	process_single_quote(const char *str, t_shell *shell, size_t *i,
		size_t *len)
{
	(*i)++;
	while (str[*i] && str[*i] != '\'')
...
{
	if (str[*i + 1] == '?')
		return (process_dollar_question(str, shell, i, len));
	else
...
int	process_quote(const char *str, t_shell *shell, size_t *i, size_t *len)
{
	int	length;
